...
class SaidaService:
        ...
        *,
        ...
        try:
            ...
            if data.data_saida is not None and data.data_saida < contexto["data_entrada"]:
                raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="A saída não pode ocorrer antes da entrada.")
            ...
            if data.tipo_saida == TipoSaidaEnum.VENDA:
                ...
                if data.preco_venda_unitario is not None:
                    raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Somente saídas do tipo VENDA podem possuir preço.")
    ...
    async def find_by_id(db: AsyncSession, saida_id: int):
        row = await SaidaRepository.find_detail_by_id(saida_id)
    ...
    async def update(db: AsyncSession, saida_id: int, data: SaidaUpdateSchema):
        ...
        try:
            ...
            if diferenca > 0:
                ...
                estoque.quantidade_atual = estoque.quantidade_atual - diferenca

            elif diferenca < 0:
                estoque.quantidade_atual = estoque.quantidade_atual + abs(diferenca)
            ...
            if tipo_final == TipoSaidaEnum.VENDA:
                if "preco_venda_unitario" in data.model_fields_set:
                    ...
                    saida.tipo_saida
                    == TipoSaidaEnum.VENDA
                    and saida.preco_venda_unitario is not None
                ):
            ...
            await SaidaRepository.update(db, saida, values)
