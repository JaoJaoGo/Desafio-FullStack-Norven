...
async def seed_admin(session: AsyncSession) -> None:
    # Evita criar o administrador novamente
    query_admin = select(UsuarioModel).where(
        UsuarioModel.email == ADMIN_EMAIL
    )
    ...
    if admin_existente:
        print("Usuário administrador já cadastrado.")
        return

    # Busca Goiânia especificamente no estado de Goiás
    query_goiania = (
        select(CidadeModel)
        .join(
            EstadoModel,
            CidadeModel.estado_id == EstadoModel.id
        )
        .where(
    ...
    if goiania is None:
        raise RuntimeError(
            "Goiânia/GO não foi encontrada. "
            "Execute o GeographySeeder antes do AdminSeeder."
        )
    ...
    print(
        f"Administrador criado com sucesso: {ADMIN_EMAIL}"
    )