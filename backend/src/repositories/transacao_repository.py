from sqlalchemy import Integer, Numeric, String, Text, cast, func, literal, select, union_all
...
class TransacaoRepository:
    ...
    def _build_combined_query():
        ...
        return union_all(entrada_query, saida_query).subquery("transacoes")
    
    @staticmethod
    async def _list(db: AsyncSession, filters: TransacaoFilterSchema, produto_id: int | None = None):
        combined = TransacaoRepository._build_combined_query()
        ...
        if filters.search:
            ...
            conditions.append(
                (combined.c.produto_nome.ilike(value))
        ...
        produto_filter = produto_id if produto_id is not None else filters.produto_id
        
        if produto_filter is not None:
            conditions.append(combined.c.produto_id == produto_filter)
    ...
    async def list(db: AsyncSession, filters: TransacaoFilterSchema):
        return await TransacaoRepository._list(db, filters)
    
    @staticmethod
    async def list_by_product(db: AsyncSession, produto_id: int, filters: TransacaoFilterSchema):
        return await TransacaoRepository._list(db, filters, produto_id=produto_id)