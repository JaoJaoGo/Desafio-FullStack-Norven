...
class TransacaoFilterSchema(SCBaseModel):
    ...
    quantidade: Optional[Decimal] = Field(default=None, ge=0)
    ...
    def validar_filtros(self):
        if self.quantidade_min is not None and self.quantidade_max is not None and self.quantidade_min > self.quantidade_max:
            raise ValueError("quantidade_min não pode ser maior que quantidade_max.")

        if self.data_inicio is not None and self.data_fim is not None and self.data_inicio > self.data_fim:
