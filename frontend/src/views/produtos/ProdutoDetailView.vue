...
export default defineComponent({
  ...
  data() {
    return {
      ...
      lotes: [] as Lote[],
      lotesDisponiveisEntrada: [] as Lote[],
  ...
  computed: {
    ...
    loteOptions() {
      return this.lotesDisponiveisEntrada.map(
        (lote) => ({
  ...
  methods: {
    async initialize(): Promise<void> {
      ...
      try {
        ...
        await Promise.all([
          this.loadLotes(),
          this.loadLotesDisponiveisEntrada(),
    ...
    async loadLotesDisponiveisEntrada(): Promise<void> {
      const produtoId = this.produtoId
      ...
      this.lotesDisponiveisEntrada = await loteService.listAllByProduct(produtoId, true)
    },
          ...
          <v-card-text class="pa-6">
            ...
            >
              <v-radio
                label="Entrada"
                value="ENTRADA"
                :disabled="lotesDisponiveisEntrada.length === 0"
              />
            ...
            <v-alert
              v-if="lotesDisponiveisEntrada.length === 0"
              type="warning"
              variant="tonal"
              density="compact"
              class="mb-5"
            >
              Não há lotes válidos disponíveis para entrada.
              Cadastre um novo lote antes de registrar uma entrada.
            </v-alert>
            ...
            <v-alert
              ...
              class="mb-5"
            >
              Não há saldo disponível para registrar saídas.
            </v-alert>
